	 ...
	 if (currentEventID != nextIterEventID ) //if next event is new: analyse and clear all vectors
	   {
	     ...
	     //  printf("%d %d %f %f %f %f %.12f \n",  v_blockID[]currentEventID, currentBlockID, currentEdep, currentX, currentY, currentZ, currentTime );
...
ema	     //float winnerYi[v_uniqueblockID.size()];
		 ...
		 for (int k=0; k<v_uniqueblockID.size();k++)
		   {
			 ...
			 */
			 // cout<<"Current "<< v_blockID[k]<<" "<< v_currentEdep[k]<<" "<< v_currentX[k] <<" "<< v_currentY[k]<<" "<< v_currentZ[k]<<endl;
			 winnerX[k] += v_currentX[j]*v_currentEdep[j];
			 winnerY[k] += v_currentY[j]*v_currentEdep[j];  
			 winnerZ[k] += v_currentZ[j]*v_currentEdep[j];
			 ...
			 //cout<<"Current "<< v_blockID[k]<<" "<< v_currentEdep[k]<<" "<<winnerX[k] <<" "<<winnerY[k]<<" "<< winnerZ[k]<<endl;
			 
			 //summ of dep energy 
			 totEdep[k] += v_currentEdep[j];
			 //cout<<"edep "<< totEdep[k]<<endl;

			 		    
			 /* winnerX[k]=winnerX[k]/totEdep[k];
			 winnerY[k]=winnerY[k]/totEdep[k];
		 ...
		 h_hits_X->Fill(winnerX[k]/totEdep[k]);
		 h_hits_Y->Fill(winnerY[k]/totEdep[k]);
		 h_hits_Z->Fill(winnerZ[k]/totEdep[k]);
		 if (totEdep[k] >0.512)
		  printf("something is wrong");
		 
		   printf("! %d %f %f %f %f %.12f\n", v_uniqueblockID[k], totEdep[k], winnerX[k]/totEdep[k], winnerY[k]/totEdep[k],winnerZ[k]/totEdep[k],  minTime[k]);
